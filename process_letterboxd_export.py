...
REVIEWS_PATH = os.path.join(DEST_DIR, 'reviews.csv')
...
if __name__ == "__main__":
    ...
    rating_results = sync_ratings_to_plex(plex_movies, lb_ratings)
    ...
    watched_results = mark_reviewed_movies_watched(plex_movies, lb_reviews)
    ...
    if deleted_movies:
        ...
        for file_path in COLLECTION_FILES:
            if os.path.exists(file_path):
                process_collection_file(file_path, lb_reviews, plex_movies)
        ...
        for title, year in deleted_movies:
            success, message = remove_movie(title, year, delete_files=True, movie_cache=radarr_cache)
            if success:
                print(f"  {message}")
                removed += 1
